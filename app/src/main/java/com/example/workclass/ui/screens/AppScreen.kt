...
fun AppScreen() {
    ...
    var eventos by remember { mutableStateOf(emptyList<String>()) }
    ...
    // Cargar contactos y eventos si es necesario
    LaunchedEffect(permisoContactosConcedido, mostrarContactos, permisoCalendarioConcedido, mostrarEventos) {
        if (permisoContactosConcedido && mostrarContactos) {
            contactos = obtenerContactos(context)
        }
        if (permisoCalendarioConcedido && mostrarEventos) {
            eventos = obtenerEventos(context)
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            Button(onClick = {
                ...
                Text(if (mostrarEventos) "Ocultar eventos" else "Mostrar eventos")
            }

            Spacer(modifier = Modifier.height(12.dp))

            if (mostrarEventos && permisoCalendarioConcedido) {
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        ...
                        LazyColumn(modifier = Modifier.heightIn(max = 300.dp)) {
                            items(eventos) { evento ->
                                Text("• $evento", modifier = Modifier.padding(4.dp))
                            }
                        }
                    }
                }
...
fun obtenerEventos(context: Context): List<String> {
    val lista = mutableListOf<String>()
    val cursor = context.contentResolver.query(
    ...
    cursor?.use {
        val tituloIndex = it.getColumnIndex(CalendarContract.Events.TITLE)
        val descripcionIndex = it.getColumnIndex(CalendarContract.Events.DESCRIPTION)
        val inicioIndex = it.getColumnIndex(CalendarContract.Events.DTSTART)
        val dateFormat = SimpleDateFormat("dd/MM/yyyy HH:mm", Locale.getDefault())

        while (it.moveToNext()) {
            val titulo = it.getString(tituloIndex) ?: "Sin título"
            val descripcion = it.getString(descripcionIndex) ?: ""
            val inicioMillis = it.getLong(inicioIndex)
            val fecha = dateFormat.format(Date(inicioMillis))
            lista.add("$titulo - $fecha${if (descripcion.isNotEmpty()) ": $descripcion" else ""}")
        }
    }
    return lista
}